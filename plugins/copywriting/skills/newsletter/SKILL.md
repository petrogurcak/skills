---
name: newsletter
description: Writes high-converting emails — newsletters, promo campaigns, onboarding sequences, cold outreach — using a 9-step email anatomy framework with subject line techniques, personalization, PROTTO methodology, and psychological triggers. Use when creating any email that needs to drive opens, clicks, or replies. Trigger phrases: "write a newsletter", "email campaign", "subject line", "email sequence", "cold email". Do NOT use for website copy (use web-copy), product descriptions (use product-copy), or social media (use instagram-content).
metadata:
  author: Petr
  version: 1.0.0
---

# Newsletter & Email Marketing

## Overview

This skill provides a comprehensive framework for writing **high-converting emails** - newsletters, direct campaigns, automated sequences. It combines Otto Bohuš email-specific techniques with PROTTO methodology, psychological triggers, and persuasion principles.

**Core principle:** 2 seconds to open (subject). 5 seconds to engage (opening). Everything else leads to ONE clear action.

---

## When to Use

Use this skill when:

- ✅ Writing newsletters (regular or promotional)
- ✅ Creating direct email campaigns
- ✅ Developing email sequences (onboarding, nurture)
- ✅ Crafting cold outreach emails
- ✅ Writing transactional emails with marketing value

**Do NOT use for:**

- ❌ Website copy (use `copywriting:web-copy`)
- ❌ Product descriptions (use `copywriting:product-copy`)
- ❌ Social media (use `copywriting:instagram-content`)

---

## Before Starting: Complete the Brief

**Strongly recommended:** Work through the [Core Briefing Process](../core-briefing-process.md) before writing email campaigns.

**Otto's Principle:**

> "Nemohu začít psát texty, dokud nemám jasné zadání"
> _(Cannot begin writing without clear direction)_

### Quick Brief for Email Campaigns

**Minimum questions to answer:**

- **KOMU (Audience):** Who are your subscribers? Segments? (new subscribers, engaged, cold, VIP)
- **JAK (Journey):** Where are they in the customer journey? Awareness stage?
- **KAM (Goal):** What action do you want? (click, reply, purchase, engagement)
- **ČÍM (Differentiation):** What value can you offer that inbox competitors can't?
- **PROČ (Why):** Why this email series? What's the deeper purpose?

**Full brief:** See [Core Briefing Process](../core-briefing-process.md) for all 8 questions (CO, JAK, ČÍM, KDE, KOMU, KDO, KAM, PROČ).

**Why this matters for Email:**
Without understanding your audience segments (KOMU), you can't personalize effectively (300% conversion boost). Without knowing their journey (JAK), your messaging will be off. Without a clear goal (KAM), your CTA will fail. Without differentiation (ČÍM), you're just inbox noise.

**Integration with PROTTO:**
The brief answers become inputs for PROTTO Step 1 (PROstuduj) - you cannot study your audience without first briefing the project.

---

## Core Copywriting Principles

**Before starting, review:** [Core Copywriting Principles](../core-copywriting-principles.md)

Key principles for email:

1. **Předmět = 50 % úspěchu** - 2 sekundy na rozhodnutí
2. **Prvních 5 sekund** rozhoduje o čtení
3. **Jeden odstavec = max 3 řádky** - Email se skenuje ještě více než web
4. **Personalizace** zvyšuje konverzi o 300 %

---

## The Process: Email Anatomy (9 Steps)

**IMPORTANT: Use TodoWrite to create todos for EACH step.**

### Step 1: Předmět (Subject Line)

- [ ] Napsat 10 verzí předmětu
- [ ] Vybrat nejlepší
- [ ] Vylepšit ještě jednou
- [ ] Otestovat na antispam

**Kritický fakt:**

> "Předmět rozhoduje o otevření či smazání e-mailu během **2 sekund**."

---

#### Co NEDĚLAT v předmětu

**❌ Opakovat název firmy:**

- "Newsletter od Firmy XYZ"
- "Novinky od nás"

**❌ Používat blokovaná slova:**

- "Sleva" / "Zdarma" / "Akce"
- "Výhra" / "Bonus" / "Gratis"
- "Klikněte zde"

**❌ Psát obecné fráze:**

- "Novinky od..."
- "Speciální nabídka"
- "Máme pro vás..."

---

#### Co DĚLAT v předmětu

**✅ Inteligentní otázka budící zvědavost:**

- "Petra, ještě jste o tom nepřemýšlela?"
- "Víte, proč vaše Instagram neprodává?"

**✅ Kontext reagující na aktuální dění:**

- "Po včerejším webináři..."
- "Petra, tohle jste minulý týden přehlédla"

**✅ Osobní oslovení s konkrétním benefitem:**

- "Petra: 3 věci, které vám ušetří 5 hodin"
- "Jak jsem za 3 dny prodala, na co jsem čekala 8 měsíc"

**✅ Vyvolání nedořečenosti:**

- "To, co se stalo v pátek..."
- "Nemůžu uvěřit, že jsem tohle našel..."
- "Nevěřila byste, kdo se zeptal..."

---

#### Techniky pro předmět

**1. Čísla a specifičnost:**

- "3 chyby, které děláte s kávou"
- "Ušetřete 2 490 Kč do pátku"

**2. Zvědavost (otevřená smyčka):**

- "Tohle by vás mohlo překvapit..."
- "Konečně jsem zjistil proč..."

**3. Personalizace:**

- "Petra, toto je pro vás"
- Lokace: "Petra (Praha), speciálně pro vás"

**4. Urgence (pozor na spam):**

- "Zbývá 24 hodin"
- "Poslední šance" (ale musí být pravda!)

**5. Negativní rámování:**

- "Přestaňte dělat tuto chybu"
- "5 věcí, co vás zdržují"

---

#### Test předmětu

**Nástroj:** Napište 10 verzí. Vyberte nejlepší. Pak ho ještě vylepšete.

**Checklist:**

- [ ] Vyvolává zvědavost?
- [ ] Není tam spam slovo?
- [ ] Max 50 znaků? (mobilní zobrazení)
- [ ] Personalizace přítomna?
- [ ] Jasný benefit nebo intriga?

**Antispam test:**
Použijte nástroje jako Mail-Tester nebo IsNotSpam.

---

### Step 2: Preheader (Preview text)

- [ ] Napsat preheader jako rozšíření předmětu
- [ ] Max 90 znaků (mobilní)
- [ ] Doplňuje předmět, ne opakuje

**Co je to:**
Text viditelný vedle předmětu v inboxu (náhled prvních slov emailu).

**Využití:**
Rozšiřte příběh z předmětu. Posílíte zvědavost.

**Příklad:**

```
Předmět: Petra, tohle jste minulý týden přehlédla
Preheader: ...a stojí to jen 7 minut vašeho času
```

**Jak nastavit:**
V HTML šabloně nebo první věta emailu (pokud nástroj neumí preheader nastavit).

---

### Step 3: Pozdrav (nebo jeho absence)

- [ ] Rozhodnout: pozdravit nebo ne
- [ ] Pokud ano: jen křestní jméno
- [ ] Pokud ne: rovnou do věci

**Problém:**

> "Klasické zdravice typu 'Vážený pane...' patří do minulosti."

**Realita:**

> "Máte pouze **5 sekund pozornosti** čtenáře."

---

#### Co NEDĚLAT

**❌ Formální pozdrovy:**

- "Vážený pane Nováku,"
- "Milá paní Svobodová,"
- "Dobrý den,"
- "Zdravím Vás,"

---

#### Co DĚLAT

**✅ Rovnou do věci:**

```
Včera jsem přemýšlel o vašem projektu a napadlo mě...
```

**✅ Nebo jen křestní jméno:**

```
Petře,

všiml jsem si, že...
```

**✅ Případně vůbec nic:**
Začít přímo textem (riskantní, ale může fungovat).

---

### Step 4: Nadpis/Hook (Eye-catching opening)

- [ ] Vytvořit veliký, vizuálně výrazný nadpis
- [ ] Zajímavý nebo provokativní
- [ ] Konkrétní příslib

**Pravidlo:**

> "Bez 'kvalitního eye-catching' nadpisu hned na úvod prohráváte."

---

#### Vlastnosti dobrého nadpisu v emailu

**1. Veliký**

- Vizuálně výrazný (H1 nebo H2 velikost)
- Tučný font
- Barva odlišná od běžného textu

**2. Zajímavý**

- Překvapivý nebo provokativní
- Vyvolává emoce
- Nečekaný úhel

**3. Konkrétní příslib**

- Co z toho budu mít
- Jasný benefit
- Měřitelný výsledek

---

#### Příklady

**❌ Špatné nadpisy:**

- "Novinky z našeho obchodu"
- "Speciální nabídka pro Vás"
- "Newsletter - únor 2025"

**✅ Dobré nadpisy:**

- "Ivo, jste opravdová umělkyně! Vaše ruční výrobky si prohlédlo přes 1 500 zájemců..."
- "Jak jsem za 3 dny prodala, na co jsem čekala 8 měsíců"
- "Tento trik vám ušetří 40 hodin měsíčně"

---

#### Technika

**Zasáhněte čtenáře přímo do srdce:**

- Emoce (radost, strach, překvapení)
- Čísla (specifické výsledky)
- Osobní oslovení (jméno)

**Triáda aplikace:**

1. **Rezonance**: "Znáte to, když..."
2. **Redefinice**: "Co kdyby problém nebyl v..., ale v..."
3. **Rezistence**: "Možná si říkáte: 'To je příliš dobré.' Rozumím..."

---

### Step 5: Tělo emailu (Body copy)

- [ ] První 2-3 odstavce max 3 řádky
- [ ] Zaměřeno na problémy čtenáře (ne o firmě!)
- [ ] Blueprint aplikován (5 otázek)
- [ ] Emoce vstříknuty

**Manttra:**

> "Hned v prvních **dvou třech KRÁTKÝCH odstavcích** ať je vymalováno."

---

#### Struktura ideálního úvodu (3 odstavce)

**1. odstavec (1-2 řádky):**
Co se děje / o co jde

**2. odstavec (2-3 řádky):**
Proč je to důležité pro čtenáře

**3. odstavec (1-2 řádky):**
Co s tím může udělat

---

#### Zlaté pravidlo

**Jeden odstavec = maximálně 3 řádky**

**Zaměření:**

- ✅ Problémy čtenáře
- ❌ NEPIŠTĚ o sobě a své firmě

---

#### Příklad transformace

**❌ Špatně (rozvláčné, o firmě):**

```
Naše společnost byla založena v roce 2010 s cílem poskytovat
kvalitní služby v oblasti marketingu. Za dobu naší existence
jsme spolupracovali s mnoha klienty a získali cenné zkušenosti,
které nyní můžeme nabídnout i Vám...
```

**✅ Dobře (krátké, o čtenáři):**

```
Petra, pamatujete si, jak jste mi říkala, že vás štve
neustálé hašení požárů v marketingu?

Našel jsem řešení. Trvá to 20 minut týdně.

Chcete vědět jak?
```

---

### Step 6: Personalizace

- [ ] Jméno příjemce
- [ ] Firma / město
- [ ] Historie nákupů / chování
- [ ] Předchozí komunikace

**Klíč:**

> "Personalizace je asi nejvíc podceňovaný marketingový fígl dneška."

**Síla:**

> "Správně personalizovaný email může zvýšit konverzi až o **300 %**."

---

#### Co personalizovat

**Základní:**

- Jméno příjemce
- Firma
- Město

**Pokročilé:**

- Historie nákupů
- Chování na webu
- Předchozí komunikace
- Zájmy a preference
- Abandoned cart
- Browsinghistory

---

#### Příklady

**❌ Špatná personalizace (jen jméno):**

```
Dobrý den {JMENO},

máme pro vás speciální nabídku...
```

**✅ Dobrá personalizace (kontext + chování):**

```
Petře,

všiml jsem si, že jste si minulý týden prohlížela
náš kurz "Instagram pro začátečníky".

Zajímalo by mě, co vás od nákupu zastavilo?

Možná by vám pomohlo, kdybyste viděla, jak kurz
funguje zevnitř. Připravil jsem pro vás ukázku
první lekce...
```

---

### Step 7: Tón a vtip

- [ ] Uvolněný, přátelský jazyk
- [ ] Jako s kamarádem
- [ ] Emoji s mírou
- [ ] Bez křečovité reklamštiny

**Princip:**

> "Neberte se (při psaní) tak vážně."

**Paradox:**

> "Občas můžete prodávat i tím, že nic neprodáváte – jen budete milí."

---

#### Příklady

**❌ Formální, nudné:**

```
Dovoluji si Vás upozornit na naší novou nabídku,
která by mohla být pro Vaši společnost přínosná...
```

**✅ Přátelské, vtipné:**

```
Hele Petře,

zrovna jsem četl váš článek o tom, jak jste zapomněla
na schůzku. Smál jsem se, protože já udělal totéž minulý
týden 😅

Napadlo mě, že by se vám mohla hodit naše appka...
```

**Tip:**
Můžete použít i emoji, ale s mírou. Ne v každé větě.

---

### Step 8: Struktura a formátování

- [ ] Formátování (bold, italic, CAPS)
- [ ] Nadpisy a mezititulky
- [ ] Odrážky pro seznamy
- [ ] Vizuální prvky (čáry, bílé místo)
- [ ] Hyperlinky s benefitem (2-3 hlavní)

**Problém:**

> "Jeden dlouhý blok textu nikdo nečte."

---

#### Řešení - používejte

**1. Formátování:**

- **Tučný text** pro důležité myšlenky
- _Kurzíva_ pro citace nebo zvýraznění
- VERZÁLKY pro výjimečné případy (ne často!)

**2. Struktura:**

- Nadpisy a mezititulky
- Odrážky pro seznamy
- Číslované kroky pro postupy

**3. Vizuální prvky:**

- Horizontální čáry pro oddělení sekcí
- Bílé místo mezi odstavci
- Krátké řádky (max 80 znaků)

**4. Hyperlinky:**

- **Prolinkujte** důležité pasáže
- Ale ne "katalog linků" - vyberte 2-3 hlavní
- Link musí obsahovat benefit (ne "klikněte zde")

---

#### Příklad transformace

**❌ Špatná struktura:**

```
Máme pro vás nabídku našeho nového produktu který je
skvělý a pomůže vám ušetřit čas a peníze a navíc je
teď se slevou a můžete si ho objednat na našem webu
kde najdete více informací...
```

**✅ Dobrá struktura:**

```
Máme pro vás 3 novinky:

1. **Nový kurz** - Instagram Masterclass
   → Začínáme 15. února

2. **Sleva 30 %** - Jen do konce týdne
   → Použijte kód: PROMO30

3. **Bonus zdarma** - Šablony pro Stories
   → Ke každému kurzu

[Chci vědět víc →]
```

**Zlaté pravidlo:**

> "Text musí mít **vizuální rytmus**. Oko čtenáře se nesmí ztratit."

---

### Step 9: CTA (Call to Action)

- [ ] Jasná výzva k akci
- [ ] Vizuálně výrazná (tlačítko/link)
- [ ] Konkrétní benefit
- [ ] Umístění (konec + polovina u delších)
- [ ] Urgence + garance

**Kritické pravidlo:**

> "Pokud chybí výzva k akci, prohráli jste. **Tečka.**"

---

#### Požadavky na CTA

**1. Jasnost:**
Přesně řekněte, co mají udělat.

❌ "Můžete se podívat na nabídku"
✅ "Klikněte sem a rezervujte si místo"

**2. Vizuální výraznost:**

- **Velký font**
- Zelené nebo oranžové tlačítko
- Dostatek bílého místa kolem
- Případně emoji pro zvýraznění

**3. Konkrétní benefit:**
Ne jen "Koupit", ale "Získejte kurz se slevou 30 %"

**4. Umístění:**

- Vždy na konci
- U delších emailů i v polovině
- Opakovaně (ale ne spam)

---

#### Příklady CTA

**❌ Špatné CTA:**

```
Více informací najdete na našem webu.
```

**✅ Dobré CTA:**

```
[CHCI UŠETŘIT 3 000 KČ →]

(Sleva platí jen do pátku 23:59)
```

**✅ Ještě lepší:**

```
🎯 ANO, CHCI SE NAUČIT INSTAGRAM ZA 30 DNÍ

✓ Sleva 30 % (ušetříte 2 490 Kč)
✓ Bonus: 50 šablon Stories zdarma
✓ Záruka vrácení peněz 14 dní

[REZERVOVAT MÍSTO SE SLEVOU →]

⏰ Zbývá 47 míst | Sleva končí za 2 dny
```

**Tip:**
Přidejte urgenci (čas, množství) a garance (vrácení peněz).

---

## PROTTO for Email Campaigns

**IMPORTANT: Use PROTTO for complex campaigns or launches.**

### When to Use PROTTO

PROTTO je vhodné pro:

- ✅ Velké kampaně (produktový launch)
- ✅ Email sekven ce (onboarding, nurture)
- ✅ Strategické newslettery

**Ne pro:**

- ❌ Jeden-off transactional emails
- ❌ Jednoduchá oznámení

---

### PROTTO Steps pro Email

**Kroky 1-3 (Příprava):**

1. **PROstuduj**: Segment, persony, historie interakce
2. **PROžij**: Empatie - problémy a touhy
3. **PROspěj**: Jasný benefit emailu

**Kroky 4-6 (Tvorba):** 4. **PRObuď**: Předmět + hook (3s pro zaujmutí) 5. **PRObodni**: Emoce, storytelling, visualizace 6. **PROlom**: Důvěryhodnost (testimonials, garance)

**Kroky 7-10 (Finalizace):**
7-10: Kontrola, zrání, testování, publikování

---

## Psychological Triggers for Email

**IMPORTANT: Apply at least 2 triggers per email.**

### 10 Triggerů aplikovaných na email

| Trigger       | Princip                         | Aplikace na email             |
| ------------- | ------------------------------- | ----------------------------- |
| **Dar**       | Reciprocita                     | Lead magnet zdarma v předmětu |
| **Důkaz**     | Sociální tlak                   | "847 lidí si stáhlo..."       |
| **Triáda**    | Rezonance-Redefinice-Rezistence | Struktura body copy           |
| **Zeigarnik** | Nedokončené                     | "Pokračování zítra" v P.S.    |
| **Emoce**     | Pravá hemisféra                 | Storytelling, vizualizace     |
| **Kotva**     | Relativní ceny                  | "Původně 5 990, teď 2 990"    |
| **Zapojení**  | Psaní = závazek                 | "Odpovězte na tento email"    |
| **Cyrano**    | Emoční obal                     | Příběh za produktem           |
| **Hypnóza**   | NLP                             | Vložené příkazy v textu       |
| **Limit**     | Nedostatek                      | "Zbývá 24 hodin"              |

---

### Dar (Reciprocita) v emailu

**Strategie:**
Dej něco zdarma PŘED žádostí o akci.

**Příklady:**

- Bezplatný e-book v úvodním emailu
- Hodnotný tip/trik
- Ukázka kurzu
- Šablony, checklists

**Pravidlo:**

> "Čím větší pecku jim při prvním setkání dáte, tím silněji si je k sobě připoutáte."

---

### Urgence a scarcity

**Urgence (časová):**

- "Sleva končí za 24 hodin"
- "Poslední šance dnes"
- Dynamické odpočítávání

**Scarcity (množstevní):**

- "Zbývá 12 míst"
- "Poslední kusy skladem"
- "Jen pro prvních 50 lidí"

**Varování:**
MUSÍ být pravda! Jinak ztratíte důvěru.

---

## Checklisty pro TodoWrite

### CHECKLIST: Pre-Send Email Review

**Předmět & Preheader:**

```
[ ] Předmět vyvolává zvědavost
[ ] Žádné spam slova
[ ] Max 50 znaků (mobilní)
[ ] Personalizace přítomna
[ ] Preheader rozšiřuje předmět (max 90 znaků)
[ ] Otestováno antispam nástrojem
```

**Úvod:**

```
[ ] Bez zbytečného pozdravu (nebo jen jméno)
[ ] Silný nadpis/hook
[ ] První 3 odstavce max 2-3 řádky
[ ] Zaměřeno na problémy čtenáře (ne o firmě)
```

**Tělo:**

```
[ ] Jeden odstavec = max 3 řádky
[ ] Personalizováno (min. jméno + kontext)
[ ] Přátelský tón (ne formální)
[ ] Dobrá struktura:
    [ ] Tučný text pro důležité myšlenky
    [ ] Odrážky pro seznamy
    [ ] Mezery mezi odstavci
    [ ] 2-3 hlavní hyperlinky s benefitem
[ ] Emoční slova místo racionálních
[ ] Storytelling prvek (pokud vhodné)
```

**CTA:**

```
[ ] Jasná výzva k akci
[ ] Vizuálně výrazné (tlačítko/link)
[ ] Konkrétní benefit v CTA
[ ] Urgence přidána (čas/množství)
[ ] Garance zmíněna (vrácení peněz)
[ ] Umístění: konec + (polovina u delších)
```

**Finální kontrola:**

```
[ ] Prošel test babičce/kamarádovi ("Rozumíš tomu?")
[ ] Všechny linky fungují
[ ] Žádné překlepy
[ ] Mobilní zobrazení zkontrolováno
[ ] Odesílatel jasně viditelný
[ ] Odhlašovací link přítomen
```

---

### CHECKLIST: Email Campaign Planning

**Před vytvořením kampaně:**

```
[ ] Cíl kampaně definován (konverze/engagement/brand)
[ ] Segment definován (komu posílám)
[ ] Persony vytvořeny nebo aktualizovány
[ ] Historie interakce prozkoumána
[ ] A/B test plánován (předmět/CTA/design)
[ ] Timing optimalizován (den/čas)
[ ] Follow-up sequence naplánována
```

**PROTTO aplikace (pro složité kampaně):**

```
[ ] PROstuduj: Segment analyzován
[ ] PROžij: Empatie mapa vytvořena
[ ] PROspěj: Benefit jasně definován
[ ] PRObuď: Hook připraven (předmět + nadpis)
[ ] PRObodni: Emoční storytelling vytvořen
[ ] PROlom: Důvěryhodnost přidána (testimonials/garance)
```

**Psychologické triggery:**

```
[ ] Minimálně 2 triggery použity:
    [ ] Dar (lead magnet)
    [ ] Důkaz (sociální)
    [ ] Triáda (struktura)
    [ ] Zeigarnik (nedořečenost)
    [ ] Urgence/Scarcity
    [ ] Personalizace (300% konverze)
```

---

## Anti-Patterns: What to Avoid

**❌ Předmět Mistakes:**

- Spam slova ("zdarma", "sleva", "akce")
- Obecné fráze ("Novinky od...")
- Příliš dlouhé (>60 znaků)
- Clickbait bez doručení slibu

**❌ Úvod Mistakes:**

- Formální pozdravy ("Vážený pane...")
- Dlouhé zdvořilostní fráze
- První odstavec o firmě (ne o čtenáři)
- Žádný hook/nadpis

**❌ Tělo Mistakes:**

- Dlouhé odstavce (>3 řádky)
- Psaní o sobě místo o problémech čtenáře
- Formální, korporátní jazyk
- Žádná personalizace
- Špatná struktura (žádné odrážky/formátování)

**❌ CTA Mistakes:**

- Chybějící CTA
- Obecné "Klikněte zde"
- Příliš mnoho CTA (rozptýlení)
- Žádná urgence

**❌ General Mistakes:**

- "Neposílejte, co byste sami smazali."
- Příliš časté posílání (spam)
- Neotestované linky
- Mobilně neoptimalizované

---

## Quick Reference: Email Types

| Typ                | Předmět    | Délka   | CTA        | Personalizace |
| ------------------ | ---------- | ------- | ---------- | ------------- |
| **Newsletter**     | Zvědavost  | Střední | 1-2        | Základní      |
| **Promo**          | Urgence    | Krátká  | 1 hlavní   | Pokročilá     |
| **Launch**         | Excitement | Dlouhá  | Opakované  | Pokročilá     |
| **Onboarding**     | Welcome    | Krátká  | 1 jasná    | Vysoká        |
| **Abandoned Cart** | FOMO       | Krátká  | 1 urgentní | Velmi vysoká  |
| **Cold Outreach**  | Personální | Krátká  | Soft ask   | Extrémní      |

---

## Integration with Other Skills

**From Product Copy:**

- Microbenefit v nadpisu
- Konkrétnost (čísla, fakta)

**From Web Copy:**

- Blueprint (5 otázek)
- Triáda (Rezonance-Redefinice-Rezistence)
- PROTTO (kroky 4-6)
- Storytelling

**From Instagram:**

- Hook techniky pro předmět
- Cliffhangery (Zeigarnik)
- Engagement CTA

---

## Real-World Impact

**Proč tato struktura funguje:**

**Předmět:**

> "50 % úspěchu. 2 sekundy na rozhodnutí. Testujte 10 verzí."

**Personalizace:**

> "300 % vyšší konverze než nepersonalizované emaily."

**Krátké odstavce:**

> "Emaily se skenují ještě více než weby. 3 řádky = max."

**CTA:**

> "Bez CTA prohráváte. Tečka. Jasné, výrazné, s benefitem."

**P.S.:**

> "P.S. je druhý nejčtenější prvek po předmětu. Použijte ho pro urgenci nebo bonusovou informaci."

---

## Supporting Files

- [Core Copywriting Principles](../core-copywriting-principles.md) - 8 základních pravidel
- [Email Templates](./email-templates.md) - šablony pro různé typy emailů (TODO: create)
- [Subject Line Library](./subject-line-library.md) - 100+ osvědčených předmětů (TODO: create)

---

## Vzorový Direct Email

```
Předmět: Petře, tohle jste minulý týden přehlédla

---

Petře,

všimla jste si, že váš Instagram má 2 500 followerů,
ale prodeje z něj máte skoro nulové?

Není to vaše chyba. Prostě nevíte TEN trik.

Připravil jsem pro vás video (7 minut), kde ukazuji:

• Proč followers neznamená prodeje
• Jak změnit profil, aby lidi KUPOVALI
• Jaký typ Stories prodává (+ 3 šablony)

Stačí jedna malá změna v bio. Seriously.

[CHCI VIDĚT VIDEO (7 MIN) →]

Cheers,
Otto

P.S. Video mažu za 3 dny, takže koukejte hned.
```

---

**Remember:** 2 seconds = subject. 5 seconds = opening. Everything else = ONE action. Test everything. Never send what you'd delete yourself.
