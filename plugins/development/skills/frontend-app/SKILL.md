---
name: frontend-app
description: Use when building web applications with Vite/TypeScript/Tailwind/Alpine.js - server-rendered apps, dashboards, admin panels, e-commerce backends. NOT for landing pages (use frontend-lp instead). Enforces DOCS-FIRST workflow.
hooks:
  PostToolUse:
...
## Router: App or Landing Page?

Building what?
├─ Web app / dashboard / admin panel / e-commerce backend → YOU ARE HERE (frontend-app)
└─ Landing page / marketing web / product showcase → Use `development:frontend-lp` instead
...
### Workflow 7: User Flow Testing (Scenario-First)

**Use for:** Any feature with user interaction

**MANDATORY CHECKLIST:**

```
☐ 1. Define user flow BEFORE implementing:
     Write in natural language:
     "User navigates to [page] → clicks [element] → sees [result]"

☐ 2. Implement the feature
     Follow Workflow 1-6 as appropriate

☐ 3. Add data-testid attributes to interactive elements:
     - Buttons: data-testid="submit-btn"
     - Inputs: data-testid="name-input"
     - Modals: data-testid="confirm-modal"
     - Lists: data-testid="item-list"

☐ 4. Write Playwright e2e test:
     - Smoke test for happy path (e2e/smoke/)
     - Core test for full journey (e2e/core/)
     - Error state test if API-dependent (e2e/error-states/)
