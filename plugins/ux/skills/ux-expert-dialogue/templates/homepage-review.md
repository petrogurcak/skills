...
**Expert provides:**
"Identifikuji [N] problémů s claimem:
...
**Doporučuji:** [#N] protože [data-backed reason]"
...
4. "Jaký je konkrétní BENEFIT pro čtenáře? (ne feature)"
5. "Five-second test: Přečtu headline 5 sekund, schovám, můžeš říct co děláme?"

**If buzzwords present:**
"Headline obsahuje buzzwordy: [list]. MarketingExperiments 2022: headlines s konkrétním benefitem konvertují +47% vs buzzwordy. Expected ztráta: ~30-40% conversion."

**If generic:**
"Test autenticity: Nahraď svoje jméno za konkurenta. Headline stále funguje? FAIL = příliš generic. Nejsi memorable, soutěžíš jen na cenu."

**If no benefit:**
"Nielsen Eye-Tracking: Users scan pro 'What's in it for me' do 10 sekund. 8 z 10 přečte headline, jen 2 pokračují (Ilinčev). Musíš chytit TEĎKA s benefitem."

**Expert provides alternatives:**
"Alternatives (ranked by expected impact):

1. **[Specific benefit + odd number + action]** → +40-50%
   - Example: 'Reduce support tickets 60% using conversational AI'
   - Why: Odd number (+21%), specific benefit, clear mechanism

2. **[Outcome-focused + emotional hook]** → +35-45%
   - Example: 'Answer customer questions instantly, 24/7 - while you sleep'
   - Why: Clear outcome, emotional ('while you sleep' = passive income dream)

3. **[Social proof + specific number]** → +30-40%
   - Example: 'SaaS teams use us to automate 70% of support'
   - Why: Relatable audience, specific number

**DOPORUČUJI: #1** protože kombinuje všech 5 charakteristik konvertujícího headlinu (odd number, benefit, length 7 words, specific, clarity > cleverness).

**Next step:** Otestuj pomocí five-second test (viz `uvp-optimization` TESTING.md) - cíl 80%+ clarity."
...
- [ ] Conversational tone (ne corporate)
...
**Example feedback:**
"Perex je generic: 'Nabízíme komplexní služby pro vaše potřeby.' Problems:

1. **No specificity** - Jaké služby? Pro které potřeby?
2. **No connection** - Headline říká X, perex mluví o Y
3. **Corporate speak** - 'Nabízíme komplexní' = buzzword bingo

Alternative:
'Za 30 minut vytvoříte landing page, která konvertuje - bez kodéra, bez designéra. Stačí odpovědět na 5 otázek o vašem produktu.'

Why better: Specific outcome (30 min), removes objections (no coder needed), clear process (5 questions)."
...
- [ ] Rychlý kontakt visible (tel/email v hlavičce)
...
**If wrong placement:**
"Kontakt je [aktuální umístění]. Nielsen eye-tracking: návštěvník webu hledající kontakt jde očima automaticky vpravo nahoru. 72% očekává top-right, 63% fallback na footer. Tvé umístění = friction. Expected impact: -15-20% contact conversions."
...
Caption placement: [ ] Under image  [ ] Above  [ ] None
```
...
- [ ] Authenticity & trust (not stock photo)
- [ ] Informational value (not just decoration)
...
Expected impact: Real photo vs stock = +35% trust (UserTesting study)"
...
Missing this = plýtváš prime engagement opportunity.

Caption example:
'Našimi klienty jsou firmy jako Google, Spotify a Airbnb' (social proof)
nebo
'Za 30 minut vytvoříte landing page, která konvertuje' (benefit reminder)"
...
- Buttons: 16-20px text, min 44x44px total size

**Challenge question:**
"Můžeš přečíst perex z 2 metrů distance?" (living room test)
...
**Impact:** Small headline = users miss it = no engagement with core message.

**Fix:** Increase to min 48px (desktop), test up to 64-72px. Expected: +25% headline readership."
...
- [ ] Využit jako konverzní prvek (not just links)
- [ ] Logicky organized (columns/groups)
...
Placement: [ ] Hero  [ ] Benefits section  [ ] Footer  [ ] Jiné
```
...
**Example feedback:**
"CTA je 'Submit'. Problems:

1. **Generic** - No benefit, unclear what happens
2. **Passive** - No action verb
...
**DOPORUČUJI: #1** pro maximum impact."

---

## SUMMARY PHASE

**After reviewing all 9 elements, expert provides:**

### Issue Summary

```
Identifikoval jsem celkem [N] problémů:

HIGH-PRIORITY (očekávaný impact >30%):
1. [ ] Headline generic/buzzwordy - Expected impact: +40-50% když fixed
2. [ ] No hero image caption - Expected impact: +10% readership
3. [ ] CTA generic "Submit" - Expected impact: +200-250%
...

MEDIUM-PRIORITY (impact 10-30%):
4. [ ] Perex too long (7 vět) - Expected impact: +15-20%
5. [ ] Newsletter no clear benefit - Expected impact: +20%
...

LOW-PRIORITY (impact <10% nebo nice-to-have):
8. [ ] Footer link organization - Expected impact: +5-10%
...
```
...
**Expected combined impact:** +150-200% improvement in above-fold conversions

---

### Medium Effort (Week 2-3)

1. Headline rewrite (with customer research - 1 day)
2. Perex simplification (2-3 hours)
3. Newsletter value prop clarification (2 hours)

**Expected combined impact:** +60-80% additional improvement
...
- **Metric:** Conversion rate (signup/purchase)
...
1. **Immediate (this week):**
   - [ ] Fix quick wins (#1-3 from list)
   - [ ] Set up A/B test for headline
   - [ ] Schedule customer interviews for research

2. **Short-term (2-3 weeks):**
   - [ ] Implement medium-priority changes
